...
/*	function $(x){
	...
	*/
	...
	function addMinistryInfo(){
		var formTag		= document.getElementsByTagName( "form" ),
			selectLi = $( 'ministryInfo' ),
			makeSelect = document.createElement( 'select' );
			makeSelect.setAttribute( "id", "ministryInfo" );
		for( var i = 0, j = ministryInfo.length; i < j; i++ ){
			var makeOption = document.createElement( 'option' );
			var optText = ministryInfo[i];
	...
	function getCheckBoxes(){
		var baptised = SS('affirm');
		if(baptised.checked){
			check1 = baptised.value;
		}
		else{
...
//Responsive Disclosure	
	$('#affirm').bind("change", function() {
		var bapDate = $('#baptismDate');
        console.log("box checked");
        SS('baptismDate').style.display = 'block';
    });
	...
	function localData(){
 	...
 	function storeData (key){
		...
		var item			= {};
		item.ministryInfo 	= ["Select Ministry:" , $('ministryInfo').value];
	...
	function editItem(){
		...
		$('ministryInfo').value = item.ministryInfo[1];
	...
	function validate(e){
		...
		var getMinistryInfo = $('ministryInfo');
		var getFname = $('fname');
		var getLname = $('lname');
		...
		errMsg.innerHTML = "";
		getMinistryInfo.style.border = "1px solid black";
	...
	var preview 			= SS('show');
	var easeVal 			= SS('range');
	var bapDate		 		= SS('affirm');
