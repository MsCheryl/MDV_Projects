...
window.addEventListener("DOMContentLoaded", function () {
	...
	//Variable defaults
	var greetingTypes = ["--Select a Greeting--", "Mr.", "Mrs.", "Ms.", "Miss"],
		sexValue,
		errMsg = ge("errors");
	...
	function makeCats() {
		var formTag = document.getElementsByTagName("form"),
			...
			makeSelect.setAttribute("id", "greeting");
		for (var i=0, j=greetingTypes.length; i<j; i++){
			var makeOption = document.createElement("option");
			var optText = greetingTypes[i];
	...
	function toggleControls(n){
		switch(n){
			case "on":
				ge("entryForm").style.display = "none";
				ge("clear").style.display = "inline";
				ge("displayLog").style.display = "none";
				ge("addNew").style.display = "inline";
				break;
			case "off":
				ge("entryForm").style.display = "block";
				ge("clear").style.display = "inline";
	...
	function storeData(key){
		...
		var logItem = {};
			...
			logItem.email = ["Email:", ge("email").value];
			logItem.date1 = ["Birthday:", ge("date1").value];
			logItem.submitTime = ["Submit Time:", ge("submitTime").value];
			logItem.sex = ["Male or Female:", sexValue];
			logItem.kids = ["Number of kids?:", ge("kids").value];
			logItem.greeting = ["Select a Greeting:", ge("greeting").value];
	...
	function getData(){
		...
		for(var i=0, len=localStorage.length; i<len;i++){
			...
			getImage(obj.greeting[1], makeSubList);
			for(var n in obj){
	...
	function getImage(treatType, makeSubList){
		...
		var setSrc = newImage.setAttribute("src", "images/"+ greetingTypes + ".png");
	...
	function editItem() {
		...
		ge("email").value = logItem.email[1];
		ge("date1").value = logItem.date1[1];
		ge("submitTime").value = logItem.submitTime[1];
		var radios = document.forms[0].sex;
		for(var i=0; i<radios.length; i++) {
		...
		ge("kids").value = logItem.kids[1];
		ge("greeting").value = logItem.greeting[1];
	...
	function validate(e){
		...
		//var getDate1 = ge("date1");
		//var getSubmitTime = ge("submitTime");
		...
		getSubmitTime.style.border = "1px solid black";
		...
		//Date validation
		if(getDate1.value===""){
			var dateError = "Please enter your Birthday.";
			getDate1.style.border = "1px solid red";
			messagesAry.push(dateError);
		}
		//Submit Time validation
		if(getSubmitTime.value===""){
			var timeError = "Please select the time.";
			getSubmitTime.style.border = "1px solid red";
